...
Predicting whether a tumor is benign or malignant from cell measurements, and finding out the hard way that the most accurate model isn't always the safest one.

## Problem
Can we predict whether a breast tumor is benign or malignant using cell measurements alone? A fast, reliable second check on a diagnosis could help catch cancer cases that might otherwise be missed or delayed.
...
- Evaluated each model with a cross-tabulation of predicted vs. actual diagnoses, focused specifically on false negatives (malignant cases predicted as benign), the error type with the most serious real-world consequences
...
## Visuals
![Accuracy by k value](charts/accuracy_by_k.png)
![False negatives by k value](charts/false_negatives_by_k.png)

## How to Run
```r
install.packages(c("class", "gmodels", "ggplot2"))
```
Then run `breast-cancer-knn-classification.R` in RStudio, or via `Rscript` from the command line.
...
- `charts/accuracy_by_k.png`, `charts/false_negatives_by_k.png` — visual comparison across k-values
